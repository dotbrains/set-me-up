...
release_status_json() {
    ...
    release_publish_plan_json
    printf '],"validated":%s,"preflight_contracts":%s,"contracts":[%s],"pushed":%s,"tagged":%s,"failed":%s,' \
        "$validated" "${preflight_contracts:-false}" "${release_contracts_json:-}" "$pushed" "$tagged" "$failed"
    release_provenance_json
    printf ',"repositories":['
    release_repositories_json
    printf ']}\n'
